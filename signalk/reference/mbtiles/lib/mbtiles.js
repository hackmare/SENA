...
function MBTiles(uri, callback) {
    ...
    // Open database synchronously but handle errors asynchronously for compatibility
    setImmediate(function() {
        try {
            // node:sqlite DatabaseSync options: readOnly (boolean), enableForeignKeyConstraints (boolean)
            var dbOptions = {};
            
            if (uri.query.mode === 'ro') {
                dbOptions.readOnly = true;
            } else if (uri.query.mode === 'rw') {
                // For rw mode, file must exist (unlike rwc which creates it)
                if (!fs.existsSync(mbtiles.filename)) {
                    return callback(new Error('SQLITE_CANTOPEN: unable to open database file'));
                }
                dbOptions.readOnly = false;
            } else if (uri.query.mode === 'rwc') {
                dbOptions.readOnly = false;
            } else {
                return callback(new Error('Only supports "ro", "rw", or "rwc" mode.'));
            }
            
            mbtiles._db = new DatabaseSync(mbtiles.filename, dbOptions);
            fs.stat(mbtiles.filename, function(err, stat) {
                if (err) return callback(err);
                mbtiles._stat = stat;
                mbtiles.open = true;
                mbtiles.emit('open', null);
                callback(null, mbtiles);
            });
        } catch (err) {
            // Wrap SQLite errors to match expected format
            if (err.message === 'unable to open database file') {
...
MBTiles.prototype._exists = function(table, callback) {
    ...
    setImmediate(function() {
        try {
            var rows = mbtiles._db.prepare(sql).all();
...
MBTiles.prototype._integrity = function(callback) {
    ...
    setImmediate(function() {
        try {
            var row = mbtiles._db.prepare('PRAGMA quick_check(1)').get();
...
MBTiles.prototype._setup = function(callback) {
    ...
    setImmediate(function() {
        try {
            mbtiles._db.exec(MBTiles.schema);
            callback();
        } catch (err) {
            callback(err);
        }
...
MBTiles.prototype.getTile = function(z, x, y, callback) {
    ...
    setImmediate(function() {
        try {
            var stmt = mbtiles._db.prepare(sql);
            var row = stmt.get(z, x, y);
            ...
            if (!row) {
                ...
                // Convert Uint8Array to Buffer if needed (node:sqlite returns Uint8Array for BLOB)
                var tileData = Buffer.isBuffer(row.tile_data) ? row.tile_data : Buffer.from(row.tile_data);
                
                var headers = tiletype.headers(tileData);
                headers['Last-Modified'] = new Date(mbtiles._stat.mtime).toUTCString();
                headers['ETag'] = mbtiles._stat.size + '-' + Number(mbtiles._stat.mtime);
                if (process.env.BRIDGE_LOG_MAX_VTILE_BYTES_COMPRESSED) {
                    var tileDataLength = tileData.length;
                    stats.count++;
                ...
                return callback(null, tileData, headers);
            }
        } catch (err) {
            // Check if it's a "no rows" type error or table doesn't exist
            // Don't mask actual database corruption errors
            if ((err.code === 'SQLITE_ERROR' || err.code === 'ERR_SQLITE_ERROR') && 
...
MBTiles.prototype.getGrid = function(z, x, y, callback) {
    ...
    setImmediate(function() {
        try {
            var stmt = mbtiles._db.prepare(sqlgrid);
            var row = stmt.get(z, x, y);
            ...
            var gridData = Buffer.isBuffer(row.grid) ? row.grid : Buffer.from(row.grid);
            
            zlib.inflate(gridData, function(err, buffer) {
                ...
                try {
                    var jsonStmt = mbtiles._db.prepare(sqljson);
                    var rows = jsonStmt.all(z, x, y);
...
MBTiles.prototype.getInfo = function(callback) {
    ...
    setImmediate(function() {
        try {
            var stmt = mbtiles._db.prepare('SELECT name, value FROM metadata');
            var rows = stmt.all();
            ...
            if (err.code === 'ERR_SQLITE_ERROR' || err.code === 'SQLITE_ERROR') {
                // Table doesn't exist, continue with empty metadata
                info.scheme = 'xyz';
                ensureZooms(info, function(err, info) {
                    if (err) return callback(err);
                    ensureBounds(info, function(err, info) {
    ...
    function ensureZooms(info, callback) {
        ...
        setImmediate(function() {
            try {
                var zooms = [];
                var stmt = mbtiles._db.prepare('SELECT zoom_level FROM tiles WHERE zoom_level = ? LIMIT 1');
...
MBTiles.prototype.startWriting = function(callback) {
    ...
    mbtiles._setup(function(err) {
        ...
        setImmediate(function() {
            try {
                mbtiles._db.exec('PRAGMA synchronous=OFF');
                callback();
            } catch (err) {
                callback(err);
            }
                ...
                case 'map':
                    ...
                    for (var id in writes[table]) {
                        ...
                        statements['map'].run(
                            ...
                            row.tile_id || null, row.zoom_level, row.tile_column, row.tile_row,
                            row.grid_id || null, row.zoom_level, row.tile_column, row.tile_row);
                    }
                    break;
                default:
                    var rows = [];
...
MBTiles.prototype.putInfo = function(data, callback) {
    ...
    setImmediate(function() {
        try {
            var jsondata;
            var stmt = mbtiles._db.prepare('REPLACE INTO metadata (name, value) VALUES (?, ?)');
...
MBTiles.prototype.getGeocoderData = function(type, shard, callback) {
    ...
    setImmediate(function() {
        try {
            var stmt = mbtiles._db.prepare('SELECT data FROM geocoder_data WHERE type = ? AND shard = ?');
            var row = stmt.get(type, shard);
            
            if (!row) return callback();
            // Convert Uint8Array to Buffer if needed
            var data = Buffer.isBuffer(row.data) ? row.data : Buffer.from(row.data);
            zlib.inflate(data, callback);
        } catch (err) {
            // Handle table not existing (SQLITE_ERROR) or other database errors
            if (err.code === 'ERR_SQLITE_ERROR') return callback();
            if (err.code === 'SQLITE_ERROR') return callback();
            return callback(err);
        }
    });
};
...
MBTiles.prototype.geocoderDataIterator = function(type) {
    ...
    var refillBuffer = function() {
        ...
        setImmediate(function() {
            try {
                var stmt = _this._db.prepare('SELECT shard, data FROM geocoder_data WHERE type = ? ORDER BY shard LIMIT ? OFFSET ?');
                var rows = stmt.all(type, chunkSize, currentPosition);
                
                rows.forEach(function(row) {
                    dataQueue.push({row: row, err: null});
                    segmentCount += 1;
                });
                ...
                refilling = false;
                dataQueue.push({row: null, err: err});
                sendIfAvailable();
            }
        });
    }
...
MBTiles.prototype.getIndexableDocs = function(pointer, callback) {
    ...
    if (pointer.table === 'carmen') {
        ...
        setImmediate(function() {
            try {
                var stmt = mbtiles._db.prepare('SELECT c.id AS id, c.text AS text, c.zxy, k.key_json FROM carmen c JOIN keymap k ON c.id = k.key_name LIMIT ? OFFSET ?');
                var rows = stmt.all(pointer.limit, pointer.offset);
                
                mbtiles.geocoderMigrateDocs(rows, function(err, docs) {
                    if (err) return callback(err);
