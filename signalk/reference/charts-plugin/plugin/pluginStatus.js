...
function composeStatus(perPath, onlineProviderCount) {
    ...
    if (perPath.length === 0) {
        ...
        chartsPart = `Found ${totalCharts} ${plural('chart', totalCharts)} from ${perPath[0].chartPath}`;
    }
    else {
        const breakdown = perPath
            .map((p) => `${p.chartPath} (${p.count})`)
            .join(', ');
        chartsPart = `Found ${totalCharts} ${plural('chart', totalCharts)}: ${breakdown}`;
    }
    const onlinePart = onlineProviderCount > 0
        ? ` + ${onlineProviderCount} online ${plural('provider', onlineProviderCount)}`
        : '';
