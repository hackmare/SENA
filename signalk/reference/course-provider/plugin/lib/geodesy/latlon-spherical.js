...
class LatLonSpherical {
    ...
    intermediatePointTo(point, fraction) {
        if (!(point instanceof LatLonSpherical))
            point = LatLonSpherical.parse(point); // allow literal forms
        if (this.equals(point))
            return new LatLonSpherical(this.lat, this.lon); // coincident points
        const φ1 = this.lat.toRadians(), λ1 = this.lon.toRadians();
        const φ2 = point.lat.toRadians(), λ2 = point.lon.toRadians();
    ...
    static intersection(p1, brng1, p2, brng2) {
        ...
        const cosθa = (Math.sin(φ2) - Math.sin(φ1) * Math.cos(δ12)) /
            (Math.sin(δ12) * Math.cos(φ1));
        const cosθb = (Math.sin(φ1) - Math.sin(φ2) * Math.cos(δ12)) /
        ...
        if (Math.sin(α1) * Math.sin(α2) < 0)
            return null; // ambiguous intersection (antipodal/360°)
        ...
        const φ3 = Math.asin(Math.min(Math.max(Math.sin(φ1) * Math.cos(δ13) +
    ...
    static crossingParallels(point1, point2, latitude) {
        ...
        const y = Math.sin(φ1) * Math.cos(φ2) * Math.cos(φ) * Math.cos(Δλ) -
    ...
    rhumbMidpointTo(point) {
        ...
        let λ3 = ((λ2 - λ1) * Math.log(f3) + λ1 * Math.log(f2) - λ2 * Math.log(f1)) /
            Math.log(f2 / f1);
        if (!isFinite(λ3))
    ...
    static areaOf(polygon, radius = 6371e3) {
        ...
        for (let v = 0; v < nVertices; v++) {
            ...
            const E = 2 *
                Math.atan2(Math.tan(Δλ / 2) * (Math.tan(φ1 / 2) + Math.tan(φ2 / 2)), 1 + Math.tan(φ1 / 2) * Math.tan(φ2 / 2));
            S += E;
        }
...
//# sourceMappingURL=latlon-spherical.js.map